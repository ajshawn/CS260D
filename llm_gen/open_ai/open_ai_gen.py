...
class EmotionSimilarGen:
    ...
    def _formulate_prompt(self, query: dict):
        ...
        prompt = f"""
## Role:
You are a natural language processing expert, skilled at generating sentences with the emotion {emotion} in Twitter style that are similar to the coreset reference.

## Task:
Given the emotion {emotion}, and coreset reference:
{coreset}

Now generate {self.n_gen_per_inference} sentences that are similar to the coreset reference while increasing the diversity of the dataset.

## Note:
1. Do not contain emojis or special characters.
...
class FewshotGen:
    ...
    def __init__(self, gen_file: str, batch_size: int, api_key: str, max_workers: int = 10, sleep_interval: int = 50, time_sleep: int = 10, **kwargs):
        ...
        self.n_gen_per_inference = kwargs.get("n_gen_per_inference", 5)
        self.emotions = kwargs.get("emotions", ["anger", "fear", "joy", "love", "sadness", "surprise"])
        self.n_shot = kwargs.get("n_shot", 20)
        self.queries = []
        self._generate_base_queries()
    ...
    def _generate_base_queries(self):
        ...
        Generate base queries for the generation task based on input data.
        """
        shots = pd.read_csv(self.gen_file)
        label2shots = {}
        for label in range(6):
            label2shots[str(label)] = shots[shots['labels'] == label].sample(n=self.n_shot)['text'].tolist()
        n_queries_per_emotion = self.n_gen // len(self.emotions) // self.n_gen_per_inference
        self.queries = [
            {"emotion": emotion, "shots": label2shots[emotion2label[emotion]]}
            for emotion in self.emotions
            for _ in range(n_queries_per_emotion)
        ]
    ...
    def _formulate_prompt(self, query: dict):
        ...
        prompt = f"""
## Role:
You are a natural language processing expert, skilled at generating more data with the emotion {emotion} in Twitter style based on the given examples to better train a model.
The given examples are the few-shot gold examples for the emotion.
Generate sentences that are diverse while simulating the real-world data distribution based on the given examples.
The final aim is to improve downstream model performance on emotion classification task.

## Task:
Given the emotion {emotion}, and the following examples:
{shots}

Now generate {self.n_gen_per_inference} sentences based on the examples to better simulate the real-world data distribution and improve model performance.
...
class ZeroshotGen:
    ...
    def _formulate_prompt(self, query: dict):
        ...
        prompt = f"""
## Role:
You are a natural language processing expert, skilled at generating more data with the emotion {emotion} in Twitter style to better train a model.
Generate sentences that are diverse while simulating the real-world data distribution to facilitate the model training process.
The final aim is to improve downstream model performance on emotion classification task.

## Task:
Given the emotion {emotion}, generate {self.n_gen_per_inference} sentences based on the examples to better simulate the real-world data distribution and improve model performance.
...
if __name__ == "__main__":
    ...
    gen_file = "data/emotion/20shot.csv"
    output_dir = "/local2/shawn/SimTrans/data/emotion"
    batch_size = 16
    max_workers = 5
    sleep_interval = 50
    time_sleep = 10
    ...
    # generator = EmotionDifferentGen(
    #     gen_file='/local2/shawn/SimTrans/tsne/coreset_data.json',
    #     batch_size=batch_size,
    #     api_key=api_key,
    #     max_workers=max_workers,
    #     sleep_interval=sleep_interval,
    ...
    # df.to_csv(os.path.join(output_dir, f"openai_{n_shot}shot_{n_gen}.csv"), index=False)
    df.to_csv(os.path.join(output_dir, "4o-zero-shot-gen.csv"), index=False)