...
def faciliy_location_order(
    ...
    for i in range(N):
        if np.max(S[i, order]) <= 0:
            continue
        temp = np.argmax(S[i, order])
        assignments[i]=class_indices[order[np.argmax(S[i, order])]]
        if weights is None:
    ...
    return class_indices[order], sz, greedy_time, S_time, assignments
...
def get_orders_and_weights(
    ...
    order_mg_all, cluster_sizes_all, greedy_times, similarity_times, assignments_all = zip(
        *map(
    ...
    assignments = np.full(N,-1,dtype=np.int32)
    for c in range(C):
        assignments[np.where(y==c)] = assignments_all[c]
    ...
    vals = order_mg, weights_mg, order_sz, weights_sz, ordering_time, similarity_time, assignments
    return vals
